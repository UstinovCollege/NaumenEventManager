...
def init_db():
    ...
    # Демо-данные (только если таблица пуста)
...
def register(mc_id):
    ...
    if request.method == 'POST':
        ...
        conn.commit()
        conn.close()
        flash("✅ Вы записаны!" if status == 'confirmed' else f"⏳ Вы в листе ожидания #{waitlist_position}", 
              "success" if status == 'confirmed' else "warning")
...
def poll_page():
    ...
    for p in polls:
        opts = conn.execute("SELECT * FROM poll_options WHERE poll_id=?", (p['id'],)).fetchall()
        p['options'] = opts
    conn.close()
...
def admin():
    ...
    if request.method == 'POST' and 'title' in request.form:
        ...
        if title and speaker and time and location and capacity:
            conn.execute("""
                ...
                VALUES (?, ?, ?, ?, ?)""", (title, speaker, time, location, capacity))
            conn.commit()
            flash("✅ Мастер-класс добавлен", "success")

    # Ответ на вопрос (если отправлена форма ответа)
    if request.method == 'POST' and 'answer_question_id' in request.form:
        ...
        if q_id and answer:
            conn.execute("UPDATE questions SET answer=?, is_answered=1 WHERE id=?", (answer, q_id))
            conn.commit()
    ...
    return render_template("admin.html", masterclasses=mcs, registrations=regs, questions=questions)
...
#УДАЛЕНИЕ УЧАСТНИКА + AUTO-WAITLIST
...
def delete_registration(reg_id):
    ...
    if reg['status'] == 'confirmed':
        ...
        next_user = conn.execute("""
            ...
            ORDER BY waitlist_position ASC LIMIT 1""", (mc_id,)).fetchone()
        if next_user:
            conn.execute("""
            ...
            conn.execute("""
                ...
                WHERE mc_id = ? AND status = 'waitlist'""", (mc_id,))
    else:
        conn.execute("""
            UPDATE registrations SET waitlist_position = waitlist_position - 1
            WHERE mc_id = ? AND waitlist_position > ?""", (mc_id, reg['waitlist_position']))
